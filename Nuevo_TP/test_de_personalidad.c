...
#define POLAR "- Polar (I) - Polar, el menor de los tres hermanos, suele estar en un segundo plano y no decir palabra. Pero aunque sea un oso de pocas palabras, no hay que subestimarlo. Con todos sus talentos ocultos, Polar es prácticamente un genio y hace todo el trabajo sucio de los osos. Tanta genialidad también hace que sea un poco raro y excéntrico."
#define PANDA "- Panda (P) - Panda es el mediano y no se parece en nada a sus hermanos. A ellos les gusta bailar; Panda se queda contra la pared. Ellos comen carne; Panda es vegetariano. Panda participa voluntariamente en casi todas las ideas de Pardo, aunque sea un poco más inteligente."
#define PARDO "- Pardo (G) - Pardo es el mayor de los hermanos. Aunque es un líder carismático, alegre y muy motivado, no es demasiado listo. A lo mejor comió pintura siendo osezno, nadie lo sabe. Pero lo que le falta de sentido común lo compensa con energía y entusiasmo desenfrenados."
...
*/
void puntos_finales(int puntos_television, int puntos_alimento, int pregunta_piso, int pregunta_grito, int* puntos){
...
*/
void resultado_test(int puntos){

    printf("tus puntos totales son %i\n",puntos);
    
    if((puntos >= 5) && (puntos <= 24)){
    printf("Tu personalidad es %s\n", POLAR);
} else if ((puntos >= 25) && (puntos <= 43)){
    printf("Tu personalidad es %s\n", PANDA);
} else {
    printf("Tu personalidad es %s\n", PARDO);
}
...
int main(){ // NO TIENE QUE HABER MAIN
